...
(function () {
  ...
  function buildClonePrompt(analysis, sourceUrl, finalUrl) {
    ...
    const lines = [
      ...
      "[L7_MOTION_SPEC] From scripts/classes: split UI(elements) vs BG(ambient)—default gentle M-fade-rise + M-button-lift on UI, M-video-raf-loop or minimal on BG unless evidence says otherwise; add scroll-scrub/GSAP only if reference implies it.Durations:nav150–200ms|section450–600ms|stagger50ms.anime.js stagger+timeline;reactbits.dev 1–2 modules→src/components/reactbits/*.prefers-reduced-motion→opacity200ms only.",
